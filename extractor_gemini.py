...
"""
    ...
    def extract(self, pdf_bytes: bytes) -> pd.DataFrame:
        ...
        self.logger.info("Taille PDF", f"{len(pdf_bytes):,} bytes")
    ...
    def _call_vision_single_page(self, image: Image.Image, page_num: int, prompt: str) -> List[Dict]:
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                err_str = str(e)
                self.logger.warning(f"Erreur page {page_num} (tentative {attempt})", err_str[:300])

                if any(x in err_str.upper() for x in ["429", "RESOURCE_EXHAUSTED", "QUOTA"]):
                    wait = attempt * 25
                    self.logger.warning(f"Rate limit → pause {wait}s")
                    time.sleep(wait)
                    continue

                if attempt == max_retries:
                    self.logger.error(f"Échec définitif page {page_num}", exc=e)
                time.sleep(2)
    ...
    def _parse_response(self, raw: str, context: str = "") -> List[Dict]:
        ...
        # Tentative regex
        match = re.search(r'\{.*"transactions"\s*:\s*\[.*?\]\s*\}', text, re.DOTALL)
        if match:
            try:
        ...
        self.logger.warning(f"Échec parsing JSON pour {context}")
        return []
    ...
    def _build_and_validate_dataframe(self, transactions: List[Dict]) -> pd.DataFrame:
        ...
        self.logger.success(f"DataFrame construit avec {len(df)} lignes")
        return df
